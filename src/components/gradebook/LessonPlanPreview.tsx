...
import type { LessonPlan, LessonQuestion, QuestionType, StudentSupports, ResourceSuggestion } from "@/types/lesson-plan";
...
export default function LessonPlanPreview({ plan, onChange, onReset }: Props) {
  ...
  const handleEnhanceLesson = async () => {
    ...
    try {
      ...
      const res = await apiPost("/lesson-ai/enhance-lesson", { lessonPlan: plan, selectedStandard });
      if (!res?.enhancedLesson) throw new Error("No enhanced lesson returned from server.");
      onChange(res.enhancedLesson);
      setLessonAiSuccess(true);
      setTimeout(() => setLessonAiSuccess(false), 8000);
    } catch (err: unknown) {
      const message = err instanceof Error ? err.message : "Enhancement failed. Please try again.";
      console.error("[lesson-ai] enhance-lesson error:", message);
      setLessonAiError(message);
    } finally {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-start justify-between gap-3 flex-wrap">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <button
            ...
            className="inline-flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-lg border border-[#1a3c5e] bg-[#1a3c5e] text-white hover:bg-[#15324f] disabled:opacity-60 disabled:cursor-wait"
          >
            {lessonAiLoading ? (
              <><Loader2 className="w-3.5 h-3.5 animate-spin" /> Enhancing Lesson…</>
            ) : (
              <><Sparkles className="w-3.5 h-3.5" /> Improve Entire Lesson with AI</>
            )}
          </button>
          <button
            type="button"
      ...
      {/* Full-lesson AI status banners */}
      {lessonAiLoading && (
        <div className="flex items-center gap-2 bg-[#1a3c5e]/5 border border-[#1a3c5e]/20 rounded-xl px-4 py-3 text-sm text-[#1a3c5e]">
          <Loader2 className="w-4 h-4 animate-spin shrink-0" />
          <span><strong>Enhancing entire lesson with AI…</strong> This may take 20–40 seconds. Do not close this page.</span>
        </div>
      )}
      {lessonAiSuccess && (
      ...
      {lessonAiError && (
        <div className="flex items-center justify-between gap-2 bg-red-50 border border-red-200 rounded-xl px-4 py-2.5 text-sm text-red-700">
          <span><strong>AI enhancement failed:</strong> {lessonAiError}</span>
          <button type="button" onClick={() => setLessonAiError(null)} className="shrink-0 text-red-400 hover:text-red-600">
            <X className="w-4 h-4" />
          </button>
        </div>
      )}

      {/* Per-support AI status banner */}
      {aiLoadingType && (
      ...
      {(plan.resourceSuggestions ?? []).length > 0 && (
        <Section title="Instructional Resource Suggestions" defaultOpen={true}>
          <p className="text-xs text-slate-500 mb-3">
            AI-generated search suggestions — clicking opens the search in a new tab. Verify resources before using with students.
          </p>
          <div className="space-y-2">
            {(plan.resourceSuggestions as ResourceSuggestion[]).map((r, i) => {
              const providerLower = r.provider.toLowerCase();
              const url = providerLower.includes("youtube")
                ? `https://www.youtube.com/results?search_query=${encodeURIComponent(r.searchQuery)}`
                : providerLower.includes("khan")
                ? `https://www.khanacademy.org/search?page_search_query=${encodeURIComponent(r.searchQuery)}`
                : null;
              const badgeColor = providerLower.includes("youtube")
                ? "bg-red-100 text-red-700"
                : providerLower.includes("khan")
                ? "bg-green-100 text-green-700"
                : "bg-slate-100 text-slate-600";
              return (
                <div key={i} className="border border-slate-200 rounded-xl p-3 bg-white flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0 space-y-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      <span className={`text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full ${badgeColor}`}>
                        {r.provider}
                      </span>
                      <span className="text-xs font-medium text-slate-700 italic">"{r.searchQuery}"</span>
                    </div>
                    <p className="text-xs text-slate-500 leading-snug">{r.purpose}</p>
                  </div>
                  {url && (
                    <a
                      href={url}
                      ...
                      className="shrink-0 inline-flex items-center gap-1 text-xs font-semibold text-[#1a3c5e] hover:text-blue-700 border border-slate-200 rounded-lg px-2.5 py-1.5 hover:bg-slate-50"
                    >
